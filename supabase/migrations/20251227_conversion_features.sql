...
CREATE TABLE IF NOT EXISTS commitments (
    ...
    commitment_tier INT REFERENCES pricing_tiers(tier_number),
    notes TEXT,
...
CREATE TABLE IF NOT EXISTS email_campaigns (
    ...
    campaign_type VARCHAR(50) NOT NULL DEFAULT 'drip', -- drip, blast, reminder
...
-- Add drip campaign tracking to existing waitlist
-- =====================================================
...
CREATE TABLE IF NOT EXISTS presale_buyers (
    ...
    tier_at_purchase INT REFERENCES pricing_tiers(tier_number),
    bonus_tokens DECIMAL(18, 2) DEFAULT 0,
    payment_method VARCHAR(50),
    transaction_hash VARCHAR(255),
    status VARCHAR(50) DEFAULT 'pending', -- pending, confirmed, completed, refunded
...
BEGIN
    ...
    WITH ranked_referrers AS (
        SELECT 
            ROW_NUMBER() OVER (ORDER BY COUNT(w2.id) DESC, w.created_at ASC) as rank,
            w.email,
            w.referral_code,
    ...
    LEFT JOIN leaderboard_rewards lr ON rr.rank >= lr.rank_start AND rr.rank <= lr.rank_end
    ORDER BY rr.rank
    LIMIT limit_count;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT rank, ref_count INTO user_rank, user_count
    FROM ranked
    ...
    IF user_rank > 1 THEN
        ...
        SELECT ref_count INTO next_rank_count
        FROM ranked
        WHERE rank = user_rank - 1;
    ELSE
        next_rank_count := user_count;
    END IF;
    
    RETURN QUERY
    SELECT 
        w.referral_code,
        COALESCE(user_count, 0::BIGINT) as referral_count,
        COALESCE(user_rank, 0::BIGINT) as current_rank,
        lr.badge_name,
        lr.badge_color,
        lr.bonus_tokens,
        (COALESCE(next_rank_count, 0) - COALESCE(user_count, 0) + 1)::INT as referrals_to_next_rank
    FROM waitlist w
    LEFT JOIN leaderboard_rewards lr ON COALESCE(user_rank, 999) >= lr.rank_start AND COALESCE(user_rank, 999) <= lr.rank_end
