...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-20 md:pt-0 overflow-hidden">
      ...
      {/* Decorative blobs - CSS only, no motion */}
      ...
      {/* Floating Crypto Icons - Pure CSS animations, hidden on mobile */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none hidden lg:block">
        ...
        {/* ETH */}
        <div 
          className="absolute animate-float-medium opacity-20"
          style={{ left: '85%', top: '15%', animationDelay: '2s' }}
        >
          <div 
            className="w-16 h-16 rounded-2xl flex items-center justify-center backdrop-blur-sm"
            style={{ 
          ...
          >
            <span className="font-bold text-sm" style={{ color: '#627EEA' }}>ETH</span>
          </div>
        </div>
        {/* SOL */}
        <div 
          className="absolute animate-float-slow opacity-20"
          style={{ left: '15%', top: '70%', animationDelay: '4s' }}
        >
          <div 
            className="w-16 h-16 rounded-2xl flex items-center justify-center backdrop-blur-sm"
            style={{ 
          ...
          >
            <span className="font-bold text-sm" style={{ color: '#00FFA3' }}>SOL</span>
          </div>
        </div>
        {/* USDC */}
        <div 
          className="absolute animate-float-medium opacity-20"
          style={{ left: '80%', top: '75%', animationDelay: '1s' }}
        >
          <div 
            className="w-16 h-16 rounded-2xl flex items-center justify-center backdrop-blur-sm"
            style={{ 
              backgroundColor: '#2775CA15',
              border: '1px solid #2775CA30',
            }}
          >
            <span className="font-bold text-sm" style={{ color: '#2775CA' }}>USDC</span>
          </div>
        </div>
      </div>
      ...
      <div className="container-main relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center py-12 lg:py-20">
          {/* Left: Text content - CSS animations only */}
          ...
          {/* Right: iPhone - Simple CSS fade in, no complex motion */}
          <div className="relative flex justify-center lg:justify-end">
            <div className="relative animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
              ...
              {/* Floating badges - CSS animation, desktop only */}
              <div 
                className="absolute -top-4 -right-6 bg-white rounded-2xl shadow-2xl p-3 border border-gray-100 hidden lg:block z-50 animate-fade-in-up"
                style={{ 
              ...
              <div 
                className="absolute -bottom-4 -left-6 bg-white rounded-2xl shadow-2xl p-3 border border-gray-100 hidden lg:block z-50 animate-fade-in-up"
                style={{ 
                  boxShadow: '0 10px 40px rgba(0,0,0,0.15)',
                  animationDelay: '1s',
                }}
