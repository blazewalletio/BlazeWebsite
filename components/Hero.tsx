...
// Floating crypto icons data - only shown on desktop
const floatingIcons = [
...
// Static iPhone component - no animations
function StaticIPhone() {
  return (
    <div className="relative">
      {/* Ambient glow */}
      <div 
        className="absolute -inset-8 rounded-[4rem] blur-3xl opacity-60"
        style={{
...
// Animated iPhone component - for desktop only
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        >
          ...
          >
            <div 
              className="relative bg-black rounded-[2.4rem] overflow-hidden"
              style={{ width: '280px', height: '590px', boxShadow: 'inset 0 0 1px 1px rgba(0,0,0,0.8)' }}
            >
              <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30">
                <div className="bg-black rounded-full" style={{ width: '120px', height: '34px', boxShadow: '0 0 0 1px rgba(255,255,255,0.05)' }}>
                  <div className="absolute left-5 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-[#1a1a2e] ring-1 ring-[#0d0d15]" />
                </div>
              </div>
              <div className="absolute inset-0">
                <Image src="/iphone_screen.jpeg" alt="BLAZE Wallet App" fill className="object-cover object-top" priority sizes="280px" />
              </div>
              <div className="absolute inset-0 pointer-events-none z-10" style={{ background: `linear-gradient(125deg, rgba(255,255,255,0.08) 0%, transparent 25%, transparent 60%, rgba(255,255,255,0.02) 100%)` }} />
            </div>
          </div>
          <div className="absolute -left-[2px] top-[100px] w-[4px] h-[28px] rounded-l-sm" style={{ background: 'linear-gradient(90deg, #5a5a5c, #7a7a7c, #5a5a5c)' }} />
          <div className="absolute -left-[2px] top-[145px] w-[4px] h-[50px] rounded-l-sm" style={{ background: 'linear-gradient(90deg, #5a5a5c, #7a7a7c, #5a5a5c)' }} />
          <div className="absolute -left-[2px] top-[205px] w-[4px] h-[50px] rounded-l-sm" style={{ background: 'linear-gradient(90deg, #5a5a5c, #7a7a7c, #5a5a5c)' }} />
          <div className="absolute -right-[2px] top-[160px] w-[4px] h-[70px] rounded-r-sm" style={{ background: 'linear-gradient(90deg, #5a5a5c, #7a7a7c, #5a5a5c)' }} />
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="absolute -top-4 -right-6 bg-white rounded-2xl shadow-2xl p-3 border border-gray-100 z-50"
      ...
      <motion.div
        ...
        className="absolute -bottom-4 -left-6 bg-white rounded-2xl shadow-2xl p-3 border border-gray-100 z-50"
        style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.15)' }}
      >
        <div className="flex items-center gap-2">
...
export default function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex items-center pt-20 md:pt-0 overflow-hidden">
      ...
      {/* Floating Crypto Icons - Desktop only */}
      {isDesktop && (
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {floatingIcons.map((icon) => (
            ...
            >
              ...
              >
                <span className="font-bold text-sm" style={{ color: icon.color }}>
                  {icon.symbol}
                </span>
              </div>
            </motion.div>
      ...
      <div className="container-main relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center py-12 lg:py-20">
          {/* Left: Text content - Static on mobile, animated on desktop */}
          ...
          {/* Right: iPhone - Static on mobile, animated on desktop */}
          <div className="relative flex justify-center lg:justify-end">
            {isDesktop ? <AnimatedIPhone /> : <StaticIPhone />}
          </div>
        </div>
      </div>
